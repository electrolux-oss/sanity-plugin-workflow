import { useClient, useWorkspace, useCurrentUser, useValidationStatus, useSchema, Preview, useFormValue, defineType, defineField, UserAvatar, useTimeAgo, TextWithTone, definePlugin, isObjectInputProps } from 'sanity';
import { jsx, jsxs, Fragment } from 'react/jsx-runtime';
import { UsersIcon, SplitVerticalIcon, CheckmarkIcon, ArrowRightIcon, ArrowLeftIcon, EditIcon, AddIcon, PublishIcon, ErrorOutlineIcon, WarningOutlineIcon, DragHandleIcon, UserIcon, ResetIcon, EarthAmericasIcon, CheckmarkCircleIcon, CircleIcon, InfoOutlineIcon } from '@sanity/icons';
import React, { useMemo, createContext, useContext, useState, useCallback, useEffect, useRef } from 'react';
import { useToast, Button, Spinner, Card, Flex, Box, Text, useClickOutside, Popover, Grid, Tooltip, useTheme, Stack, MenuButton, Menu, MenuItem, Badge, Container } from '@sanity/ui';
import { UserSelectMenu, useListeningQuery, Feedback } from 'sanity-plugin-utils';
  ...
  useEffect(() => {
    ...
    async function getUsersWithRoles() {
      try {
        ...
        const userIds = aclData.filter(user => {
...
var _a$2;
function useWorkflowDocuments(schemaTypes, filterOptions) {
  var _a2;
  const toast = useToast();
  const client = useClient({
    apiVersion: API_VERSION
  });
  const localeFilter = ((_a2 = filterOptions == null ? void 0 : filterOptions.locales) == null ? void 0 : _a2.length) ? filterOptions.locales : [];
  const QUERY = groq(_a$2 || (_a$2 = __template$2(['*[_type == "workflow.metadata" && (!defined($localeFilter) || count($localeFilter) == 0 || locale in $localeFilter)]|order(orderRank){\n    "_metadata": {\n      _rev,\n      assignees,\n      documentId,\n      state,\n      orderRank,\n      "draftDocumentId": "drafts." + documentId,\n    }\n  }{\n    ...,\n    ...(\n      *[_id == ^._metadata.documentId || _id == ^._metadata.draftDocumentId]|order(_updatedAt)[0]{ \n        _id, \n        _type, \n        _rev, \n        _updatedAt \n      }\n    )\n  }'])));
  const {
    data,
    loading,
    error
  ...
  const move = React.useCallback(async (draggedId, destination, states, newOrder) => {
    ...
    const newLocalDocuments = localDocuments.map(item => {
      var _a3;
      if (((_a3 = item == null ? void 0 : item._metadata) == null ? void 0 : _a3.documentId) === draggedId) {
        return {
          ...item,
          _metadata: {
            ...item._metadata,
    ...
    const document = localDocuments.find(d => {
      var _a3;
      return ((_a3 = d == null ? void 0 : d._metadata) == null ? void 0 : _a3.documentId) === draggedId;
    });
    ...
    await client.patch("workflow-metadata.".concat(documentId)).ifRevisionId(_rev).set({
      ...
      var _a3, _b;
      toast.push({
        title: newState.id === document._metadata.state ? 'Reordered in "'.concat((_a3 = newState == null ? void 0 : newState.title) != null ? _a3 : newStateId, '"') : 'Moved to "'.concat((_b = newState == null ? void 0 : newState.title) != null ? _b : newStateId, '"'),
        status: "success"
      });
      return res;
    }).catch(err => {
      ...
      toast.push({
        title: 'Failed to move to "'.concat((_a3 = newState == null ? void 0 : newState.title) != null ? _a3 : newStateId, '"'),
...
async function moveDraftToPublished(client, publishedId, toast) {
  const draftId = "drafts.".concat(publishedId);
  try {
    const draftDoc = await client.getDocument(draftId);
    if (!draftDoc) {
      toast.push({
        status: "error",
        title: "No draft document found with _id:', ".concat(draftId)
      });
      return;
    }
    ...
    toast.push({
      status: "success",
      title: "Document published successfully with _id:', ".concat(publishedId)
    });
    await client.delete(draftId);
    toast.push({
      status: "success",
      title: "Draft document deleted successfully::', ".concat(draftId)
    });
  } catch (error) {
...
function Filters(props) {
  ...
  const schema = useSchema();
  const onAddUserToFilter = useCallback(id => {
    if (!selectedUserIds.includes(id)) {
      toggleSelectedUser(id);
    }
  }, [selectedUserIds, toggleSelectedUser]);
  const onRemoveUserFromFilter = useCallback(id => {
    if (selectedUserIds.includes(id)) {
      toggleSelectedUser(id);
    }
  }, [selectedUserIds, toggleSelectedUser]);
  ...
  const sortedLocales = userLocales == null ? void 0 : userLocales.sort((a, b) => {
    const aSelected = selectedLocales.includes(a) ? -1 : 1;
    const bSelected = selectedLocales.includes(b) ? -1 : 1;
    return aSelected - bSelected;
  });
  const UserFilter = () => /* @__PURE__ */jsx(Card, {
    tone: "default",
  ...
  const SlimUserFilter = () => /* @__PURE__ */jsx(Card, {
    tone: "inherit",
    children: /* @__PURE__ */jsxs(Flex, {
      gap: 2,
      ...
      }), uniqueAssigneesNotMe.map(user => /* @__PURE__ */jsx(Button, {
        padding: 0,
  ...
  const LocaleFilter = () => /* @__PURE__ */jsx(Card, {
    tone: "default",
    children: /* @__PURE__ */jsx(MenuButton, {
      button: /* @__PURE__ */jsx(Button, {
        padding: 3,
      ...
      menu: /* @__PURE__ */jsxs(Menu, {
        ...
        children: [/* @__PURE__ */jsx(MenuItem, {
          ...
          padding: 3,
          text: "Toggle all"
        }), sortedLocales == null ? void 0 : sortedLocales.map((locale, idx) => {
          const selected = selectedLocales.includes(locale);
          return /* @__PURE__ */jsx(MenuItem, {
            text: new Intl.DisplayNames("en", {
              type: "language"
            }).of(locale),
            icon: selected ? /* @__PURE__ */jsx(CheckmarkCircleIcon, {
              style: {
  ...
  return /* @__PURE__ */jsx(Card, {
    ...
    children: /* @__PURE__ */jsxs(Flex, {
      ...
      children: [/* @__PURE__ */jsxs(Flex, {
        ...
        gap: 4,
        flex: 1,
        children: [shouldDisplayUserFilter && /* @__PURE__ */jsx(UserFilter, {}), !shouldDisplayUserFilter && /* @__PURE__ */jsx(SlimUserFilter, {}), userLocales && /* @__PURE__ */jsx(LocaleFilter, {})]
...
function WorkflowTool(props) {
  ...
  const [patchingIds, setPatchingIds] = useState([]);
  const [draggingFrom, setDraggingFrom] = useState("");
  const [undroppableStates, setUndroppableStates] = useState([]);
  const [selectedSchemaTypes, setSelectedSchemaTypes] = useState(schemaTypes);
  const toast = useToast();
  ...
  const userRoleNames = useMemo(() => {
    var _a2;
    return ((_a2 = user == null ? void 0 : user.roles) == null ? void 0 : _a2.length) ? user.roles.map(r => r.name) : [];
  }, [user]);
  const filterOptions = useMemo(() => filters == null ? void 0 : filters(user), [filters, user]);
  const userLocales = filterOptions == null ? void 0 : filterOptions.locales;
  ...
  const [selectedLocales, setSelectedLocales] = useState(() => {
    ...
    return (_b2 = (_a2 = filterOptions == null ? void 0 : filterOptions.locales) == null ? void 0 : _a2[0]) != null ? _b2 : [];
  });
  ...
  const {
    workflowData,
    operations
  } = useWorkflowDocuments(schemaTypes, passedFilter);
  const {
    data,
    loading,
    error
  ...
  const handleDragEnd = useCallback(async result => {
    ...
    if (!destinationStateItems.length) {
      ...
      const lastItemOrderRank = (_d = (_c = [...destinationStateItems].pop()) == null ? void 0 : _c._metadata) == null ? void 0 : _d.orderRank;
  ...
  const toggleLocales = useCallback(locales => {
    setSelectedLocales(prev => {
      const updatedLocales = Array.from(locales.reduce((set, locale) => {
        if (set.has(locale)) {
          set.delete(locale);
        } else {
          set.add(locale);
  ...
  const [selectedUserIds, setSelectedUserIds] = useState(uniqueAssignedUsers.map(u => u.id));
  ...
  const toggleSelectedSchemaType = useCallback(schemaType => {
    setSelectedSchemaTypes(prev => prev.includes(schemaType) ? prev.filter(u => u !== schemaType) : [...prev, schemaType]);
  }, []);
  const [invalidDocumentIds, setInvalidDocumentIds] = useState([]);
  const toggleInvalidDocumentId = useCallback((docId, action) => {
    ...
    children: [/* @__PURE__ */jsx(Verify, {
      ...
      children: /* @__PURE__ */jsx(Grid, {
        ...
        children: states.map((state, stateIndex) => {
          ...
          const documentCount = filterItemsAndSort(data, state.id, selectedUserIds, selectedSchemaTypes).length;
          return /* @__PURE__ */jsx(Card, {
              ...
              children: [/* @__PURE__ */jsx(StateTitle, {
                ...
                children: /* @__PURE__ */jsx(Droppable, {
                  ...
                  children: (provided, snapshot) => /* @__PURE__ */jsxs(Card, {
                    ...
                    children: [loading && /* @__PURE__ */jsx(Flex, {
                    ...
                    }), !loading && selectedLocales.length === 0 && /* @__PURE__ */jsx(Flex, {
                      align: "center",
                      justify: "center",
                      height: "fill",
                      padding: 6,
                      children: /* @__PURE__ */jsxs(Text, {
                        size: 3,
                        align: "center",
                        children: ["No documents found.", /* @__PURE__ */jsx("br", {}), " Have you selected a language in the filter?"]
                      })
                    }), selectedLocales.length > 0 && /* @__PURE__ */jsx(DocumentList, {
                      data,
                      invalidDocumentIds,
                      patchingIds,
                      selectedSchemaTypes,
